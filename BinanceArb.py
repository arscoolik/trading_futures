...
class BinanceArbBot:
    def __init__(self, exchange: ccxt.binance, coin: str, future_date: str, coin_precision: float,
                 ...
                 amount: float, num_maximum: int, threshold: float, negative_threshold: float, required_iterations: int, max_trial: int, futures_leverage: int, api_key: str, secret_key: str, debug_enabled: bool):
    ...
    def binance_future_place_order(self, symbol: str, direction: str, amount: int, type: str):
        ...
        self.logger.debug(f'({self.exchange.id}) Future orders SUCCESS: {direction} > {symbol} > {amount}')
        self.logger.debug(f'Order info: {str(order_info)}')
    ...
    def open_position(self, last_spread, tim, k = 6):
        ...
        while True:
            ...
            if r > last_spread / 2 * (1 + self.threshold * 10 * (k - delta_hours) / 6):
                ...
                while (self.exchange.fetch_balance()[self.coin]['free'] < spot_amount):
                    self.logger.warning(f'Insufficient {self.coin} balance on spot account, required: ({spot_amount})')
                    time.sleep(3)
                ...
                coin_bid1 = float(self.exchange.dapiPublicGetTickerBookTicker(params={'symbol': self.future_symbol['type1']})[0]['askPrice'])
                futures_contract_num = math.floor(spot_amount * coin_bid1 * self.futures_leverage / self.multiplier[self.coin]) - 1
                params = {
                ...
                r = coin_bid1 / spot_ask1 - 1
                print(f"OPENING FUTURES SHORT: coin_bid1 = {coin_bid1}, self.multiplier[self.coin] = {self.multiplier[self.coin]}")
    ...
    def close_position_utils(self):
        ...
        while True:
            self.logger.info(f'Currently trading {self.amount}$ of {self.coin}')
            spot_sell_price = float(self.exchange.publicGetTickerBookTicker(params={'symbol': self.spot_symbol['type1']})['bidPrice'])
            future_buy_price = float(self.exchange.dapiPublicGetTickerBookTicker(params={'symbol': self.future_symbol['type1']})[0]['askPrice'])
            
            self.logger.info(f'Initial spot price: {initial_spot_price}\nCurrent spot price: {spot_sell_price}')
            self.logger.info(f'Initial future price: {initial_future_price}\nCurrent future price: {future_buy_price}')
            calculated_profit = self.calculate_profit(initial_spot_price, spot_sell_price, initial_future_price, future_buy_price, 0.0005, self.amount, self.futures_leverage)
            self.logger.info(f'Calculated possible profit: {calculated_profit * self.amount}$')
            ...
            k_parameter = self.calculate_k_hour_parameter(profit) if profit < 0 else 6
    ...
    def calculate_profit(self, initial_spot_price, final_spot_price, initial_future_price, final_future_price, futures_fee, amount, leverage):
        spot_coin_num = amount / initial_spot_price
        futures_coin_num = math.floor(spot_coin_num * leverage * initial_future_price / 10) * 10 / initial_future_price * (1 - futures_fee)
        futures_coin_num_new = ((initial_future_price - final_future_price) / initial_future_price + 1) * futures_coin_num * (1 - futures_fee)
        spot_coin_num_new =  math.floor(spot_coin_num + futures_coin_num_new - futures_coin_num)
